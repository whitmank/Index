...
// (formats/book.ts) — a pdf is a graph of numbered objects with its index
// at the *end*. So this reads the probe's head and its bounded tail and
// works over what those contain: the trailer, the objects near it, and
// anything inside an object or metadata stream it can inflate. A file
// that keeps its metadata somewhere in the unread middle reports nothing,
// which is the same answer every other derivation gives when it can't see
// (derivations.ts) — never an error, just less to go on.
//
// The keys are this module's vocabulary, not any schema's, exactly as in
// book.ts: `title`, `author`, `published`, `genre`, `publisher`,
// `publication`, `doi`, `isbn`, `pages`, `description`. Joining them to
// whatever a user's type actually calls those fields is extract.ts's job.
 ...
 * here, because the classifier is a first guess the user overrules
 * (classify.ts), not a fact.
 */
const BOOK_PAGES = 50;
 ...
 * book.ts makes the same trade, for the same reason.
 */
 ...
 * (as in book.ts — files are not consistent about it).
 *
 ...
 */
async function read(probe: Probe): Promise<PdfMetadata> {
  const metadata: PdfMetadata = { authors: [], subjects: [] };
  const objects = new Map<number, PdfObject>();
  const dicts: Dict[] = [];
  const loose: string[] = [];

  for (const region of await regionsOf(probe)) {
...
const READINGS = new WeakMap<Probe, Promise<PdfMetadata>>();

/**
 * What this pdf says about itself, read once per probe: classification
 * asks first to decide what kind of thing the file is, extraction asks
 * second for the same answer's contents, and a probe is a handle that
 * opens its resource once (probe.ts). Never rejects — an unreadable pdf
 * reports nothing, the way an unreadable epub does.
 */
export function readPdf(probe: Probe): Promise<PdfMetadata> {
  const reading = READINGS.get(probe);
  if (reading) return reading;

  const started = read(probe).catch(() => NOTHING);
  READINGS.set(probe, started);
  return started;
}

/**
 * Which kind of document a pdf is — the ladder classify.ts hands the
 * pdf rung to, first match winning as everywhere else.
 *
...
/**
 * The same reading, as observations. Order is the v1 convention book.ts
 * set — what a thing is called, who made it, when, what it is about —
 * with the pdf-only facts after, and the description last because it is
 * the one that can run to a paragraph.
 *
 * A value the file gave several of stays several (`list`), the way
 * Dublin Core's repeated elements do in an epub: the schema join widens
 * a lone value for a `list` field and joins several back for a `string`
 * one, and only the reader knows which it actually found.
 */
export async function extractPdf(probe: Probe): Promise<Observation[]> {
  const metadata = await readPdf(probe);
  const observations: Observation[] = [];

  const say = (key: string, values: string[]) => {
    if (values.length === 1) observations.push({ key, value: values[0] as string, kind: "string" });
    else if (values.length > 1) observations.push({ key, value: values, kind: "list" });
  };
  const said = (key: string, value: string | undefined) => say(key, value ? [value] : []);

  said("title", metadata.title);
  say("author", metadata.authors);
  if (metadata.published) {
    observations.push({ key: "published", value: metadata.published, kind: "date" });
  }
  say("genre", metadata.subjects);
  said("publisher", metadata.publisher);
  said("publication", metadata.publication);
  said("doi", metadata.doi);
  said("isbn", metadata.isbn);
  if (metadata.pages !== undefined) {
    observations.push({ key: "pages", value: String(metadata.pages), kind: "number" });
  }
  said("description", metadata.description);
