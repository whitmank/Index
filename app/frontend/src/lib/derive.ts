...
import type { Connection, FieldKind, Format, Item, Resource, Schema } from "@index/database/types";
...
/** A type's own field: its name, and the kind its value editor should
...
/** The fields a type's schema pulls out of the generic list into their
 * own always-present, editable rows — everything but the identity field
 * (already drawn as the item's name, above these) and anything marked
 * `hidden`. An untyped item, one whose type names no schema, or one with
 * nothing left after both filters gets none: the generic FieldsEditor
 * (which excludes whatever this returns) draws everything instead. */
export function knownFieldsFor(type: string | null, schemas: Schema[]): KnownField[] {
  ...
  return declared.map((field) => ({ name: field.name, kind: field.kind }));
}
